...
class ProductRepository extends BaseEntityRepository
{
    ...
    protected $dql = [
        'getAllIndexed' =>
            'SELECT p FROM MxcDropshipIntegrator\Models\Product p INDEX BY p.icNumber',

        'getLinkedProductIds' =>
            'SELECT DISTINCT p.id FROM MxcDropshipIntegrator\Models\Product p '
            . 'JOIN p.variants v '
        ...
        'getArticlesWithoutProduct' =>
            'SELECT DISTINCT a FROM Shopware\Models\Article\Article a '
            . 'JOIN a.details d '
            . 'LEFT JOIN MxcDropshipIntegrator\Models\Variant v WITH v.number = d.number '
            . 'WHERE v.id IS NULL',
        ...
        'getArticle' =>
            'SELECT DISTINCT a FROM Shopware\Models\Article\Article a '
            . 'JOIN a.details d '
            . 'JOIN MxcDropshipIntegrator\Models\Variant v WITH v.number = d.number '
            . 'JOIN MxcDropshipIntegrator\Models\Product p WITH v.product = p.id '
        ...
        'getProductIdsByOptionIds'   =>
            ...
            . 'JOIN p.variants v '
            . 'JOIN v.options o '
            . 'JOIN MxcDropshipIntegrator\Models\Group g WITH o.icGroup = g.id '
        ...
        'getProductsWithReleaseDate' =>
            'SELECT p FROM MxcDropshipIntegrator\Models\Product p WHERE p.releaseDate IS NOT NULL',
        ...
        'getProductsWithFlavorSet' =>
            'SELECT p FROM MxcDropshipIntegrator\Models\Product p INDEX BY p.icNumber WHERE p.flavor IS NOT NULL',

        'getProductsWithFlavorMissing' =>
            'SELECT p.name FROM MxcDropshipIntegrator\Models\Product p INDEX BY p.icNumber WHERE (p.flavor IS NULL OR p.flavor = \'\') '
            . 'AND p.type IN (\'AROMA\', \'SHAKE_VAPE\', \'LIQUID\') AND p.name NOT LIKE \'%Probierbox%\'',

        'getProductsWithDosageMissing' =>
            'SELECT p.name FROM MxcDropshipIntegrator\Models\Product p INDEX BY p.icNumber '
            . 'WHERE (p.dosage IS NULL OR p.dosage = \'\') AND p.type = \'AROMA\'',

        'removeOrphaned' =>
            'SELECT p FROM MxcDropshipIntegrator\Models\Product p WHERE p.variants IS EMPTY',

        'setStateByIds' =>
            'UPDATE MxcDropshipIntegrator\Models\Product p SET p.:state = :value WHERE p.id IN (:ids)',

        // DQL does not support parameters in SELECT
        'getProperties'  =>
            'SELECT :properties FROM MxcDropshipIntegrator\Models\Product p INDEX BY p.icNumber',

        // DQL does not support parameters in SELECT
        'getPropertiesById'  =>
            'SELECT :properties FROM MxcDropshipIntegrator\Models\Product p WHERE p.id = :id',
        ...
        'getExcelExportMapping' =>
            ...
            . 'FROM MxcDropshipIntegrator\Models\Product p',

        'getExcelExportAroma' =>
            'SELECT p.icNumber, p.type, p.supplier, p.brand, p.name, p.dosage '
            . 'FROM MxcDropshipIntegrator\Models\Product p WHERE p.type = \'AROMA\'',
    ...
    protected $sql = [
        'updateLinkState' =>
            'UPDATE s_mxcbc_dsi_product p '
            . 'JOIN s_mxcbc_dsi_variant v ON v.product_id = p.id '
